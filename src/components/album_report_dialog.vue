<template>
  ...
  <v-layout row wrap class="ys-dialog ys-album-report-dialog">
    <v-flex xs5 px-3 py-4 class="left-pane">
      ...
      <v-divider></v-divider>
      <p>
        Yousound reviews all reports to verify if users are in violation of our
        <b>Terms of Service</b>. Accounts falsely reporting content may have
    ...
    <v-flex xs7 class="right-pane">
      <div v-if="active_violation != 'infringe'">
        <form @submit.prevent="submit">
          <v-flex x12 pa-3>
            ...
            <p>
              Yousound reviews all reports to verify if users are in violation
              of our <b>Terms of Service</b>.<br />Accounts falsely reporting
      ...
      <div v-else>
        <v-flex x12 pa-3>
          <h3>Specify the violation</h3>
          <p>
            Yousound reviews all reports to verify if users are in violation of
            our <b>Terms of Service</b>.<br />Accounts falsely reporting content
            may have their accounts terminated.
          </p>
          <v-btn dark round color="blue" class="mt-3 px-3"
            >File Copyright Claim</v-btn
          >
        </v-flex>
      </div>
    </v-flex>
    <div v-if="show_confirm_view" class="confirm-view">
      ...
      <div>
        <label>Issue:</label><span>{{ violation.label }}</span>
      </div>
      <p>
        If we find this content to be in violation of our
        <b>Term of Services</b>, we will remove it.
      </p>
      <v-btn @click="closeConfirmView()" dark round color="blue" class="px-4"
        >Close</v-btn
      >
    </div>
  </v-layout>
...
export default {
    ...
    return {
      ...
      violations: [
        ...
        },
        {
          ...
          options: ['Physical attack', 'Animal abuse'],
        },
        {
  ...
  computed: {
    violation() {
      return (
        _.find(this.violations, (v) => v.value === this.active_violation) || {
          options: [],
        }
      )
    },
