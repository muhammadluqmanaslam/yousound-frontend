    ...
    <v-layout row wrap class="popup-section">
      <v-flex xs12 class="profile-section">
        ...
        <router-link :to="'/' + trackUser.slug">
          <div
            class="avatar-image"
            :style="{'background-image': 'url(' + trackUser.avatar.thumb.url + ')'}"
          ></div>
        </router-link>
        <router-link :to="'/' + trackUser.slug">
        ...
        <v-btn
          ...
          class="follow-btn"
          :class="{'follow': !trackUser.is_following, 'following': trackUser.is_following}"
...
<script type="text/javascript">
  ...
  export default {
    ...
    methods: {
      ...
      followUser () {
        if (this.trackUser.is_following) {
          UserService.unfollowUser(this.trackUser.id).then(response => {
            this.$store.dispatch('error/showSuccessToast', ['You just unfollowed ' + this.trackUser.display_name])
          ...
          UserService.followUser(this.trackUser.id).then(response => {
            this.$store.dispatch('error/showSuccessToast', ['You just followed ' + this.trackUser.display_name])
      ...
      sendLove (token) {
        ...
        UserService.donateMoney(this.trackUser.slug, params).then(response => {
