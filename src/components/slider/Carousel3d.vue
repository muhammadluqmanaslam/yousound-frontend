...
export default {
  ...
  computed: {
    ...
    slideWidth() {
      ...
      this.size = screenHeight - 150 - 170 - 150
      const sw = parseInt(this.size) + parseInt(this.border, 10) * 2
      if (vw <= 0) {
        vw = sw
      }
      return vw < sw ? vw : sw
    ...
    slideHeight() {
      var screenHeight = $(window).height()
      this.size = screenHeight - 150 - 170 - 150
      const sw = parseInt(this.size, 10) + parseInt(this.border, 10) * 2
      const sh = parseInt(parseInt(this.size) + this.border * 2, 10)
  ...
  methods: {
    ...
    moveSlide() {
      this.goSlide(this.$store.state.player.listIndex)
    },
  },
