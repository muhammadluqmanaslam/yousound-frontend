<template>
  <v-flex class="track-card dflex" :class="{playing: isPlaying }">
    <v-flex
      v-if="!isEmptyAlbum"
        ...
        <template v-else>
          ...
          <!-- <div class="track-image" v-lazy:background-image="item.cover.url"></div> -->
			...
			<v-flex class="dflex align-center">
				...
				<div class="three-dots" v-if="meatBall"><img src="../assets/three-dots.svg" width="15"></div>
			</v-flex>
		</v-flex>

		<v-dialog
		v-model="showFeatureModal"
		content-class="feature-modal"
		>
		<featureModal :album="item" />
