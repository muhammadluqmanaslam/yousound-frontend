...
export default {
  ...
  getPlaylist(playlistId) {
    return Vue.http.get(API_BASE_URL + '/' + playlistId, {
      headers: { Authorization: $store.state.auth.token },
    })
  },

  deletePlaylist(playlistId) {
    return Vue.http.delete(API_BASE_URL + '/' + playlistId, {
      headers: { Authorization: $store.state.auth.token },
    })
  },

  updatePlaylist(playlistId, params) {
    return Vue.http.patch(API_BASE_URL + '/' + playlistId, params, {
      headers: { Authorization: $store.state.auth.token },
    })
  },
  ...
  removeTrack(playlistId, params) {
    return Vue.http.post(
      API_BASE_URL + '/' + playlistId + '/remove_track',
      params,
      { headers: { Authorization: $store.state.auth.token } }
    )
  },
