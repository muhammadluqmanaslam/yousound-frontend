...
export default {
  ...
  makePublicComment(commentId) {
    return Vue.http.get(API_BASE_URL + '/' + commentId + '/make_public', {
      headers: { Authorization: $store.state.auth.token },
    })
  },

  makePrivateComment(commentId) {
    return Vue.http.get(API_BASE_URL + '/' + commentId + '/make_private', {
      headers: { Authorization: $store.state.auth.token },
    })
  },

  deleteComment(commentId) {
    return Vue.http.delete(API_BASE_URL + '/' + commentId, {
      headers: { Authorization: $store.state.auth.token },
    })
  },

  editMessage(commentId, params) {
    return Vue.http.patch(API_BASE_URL + '/' + commentId, params, {
      headers: { Authorization: $store.state.auth.token },
    })
  },
}
