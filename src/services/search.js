...
export default {
  searchStream(params) {
    return Vue.http.post(API_BASE_URL_V1 + '/search_stream', params, {
  ...
  searchStreamV2(params) {
    return Vue.http.post(API_BASE_URL_V2 + '/search_stream', params, {
      headers: { Authorization: $store.state.auth.token },
    })
  },

  searchDiscover(params) {
    return Vue.http.post(API_BASE_URL_V1 + '/search_discover', params, {
      headers: { Authorization: $store.state.auth.token },
    })
  },
  ...
  searchLanding(params) {
    return Vue.http.post(`${API_BASE_URL_V1}/search_landing`, params, {
      headers: { Authorization: $store.state.auth.token },
    })
  },
}
