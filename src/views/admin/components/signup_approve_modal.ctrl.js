...
export default {
  ...
  computed: {
    ...
    userSubGenreName () {
      ...
      if (sub_genre_id) {
        const sub_genre = _.find(this.sub_genres, (g) => { return g.id == sub_genre_id})
    ...
    userReleasedAlbumsCount () {
      return this.getNameById(UserReleasedAlbumsCountOptions, this.item.released_albums_count)
    },

    userYearsSinceFirstReleased () {
      return this.getNameById(UserYearsInBusinessOptions, this.item.years_since_first_released)
    },

    userAnnualIncomeOnMerchSales () {
      return this.getNameById(UserAnnualIncomeOnMerchSalesOptions, this.item.annual_income_on_merch_sales)
    },

    userAnnualPerformancesCount () {
      return this.getNameById(UserAnnualPerformancesCountOptions, this.item.annual_performances_count)
    },

    userPerformanceRightsOrganization () {
      return this.getNameById(UserPerformanceRightsOrganizationOptions, this.item.performance_rights_organization)
    },
    ...
    userEmployeesCount () {
      return this.getNameById(UserEmployeesCountOptions, this.item.employees_count)
    },

    userYearsInBusiness () {
      return this.getNameById(UserYearsInBusinessOptions, this.item.years_in_business)
    },

    userProductsCount () {
      return this.getNameById(UserProductsCountOptions, this.item.products_count)
    },
    ...
    userFullAddress () {
      return `${_.get(this.item, 'city', '')}, ${_.get(this.item, 'country', '')}`
    }
  },
  ...
  methods: {
    getNameById (options, id) {
      return _.get(_.find(options, (o) => (o.id == id)), 'name', '')
    }
  }
}
