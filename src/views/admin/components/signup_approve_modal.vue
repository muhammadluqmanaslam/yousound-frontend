    ...
    <v-layout row wrap class="popup-section">
      ...
      <v-layout row wrap mx-3 py-2 class="border-bottom">
        ...
        <v-flex xs12 sm12>
          ...
          <a
            v-if="item.social_user_id"
            :href="`https://twitter.com/intent/user?user_id=${item.social_user_id}`"
            target="_blank"
        ...
        <v-flex xs12 sm12>
          ...
          <a
            ...
            :href="item.website_1_url"
            target="_blank"
            >{{ item.website_1_url }}</a
          >
        </v-flex>
        <v-flex xs12 sm12 v-if="item.website_2_url">
          <label class="link-title">Website: </label>
          <a
            v-if="item.website_2_url"
            :href="item.website_2_url"
            target="_blank"
            >{{ item.website_2_url }}</a
          >
        </v-flex>
      </v-layout>
      <v-layout
        v-if="item.request_role == 'artist'"
        row
        wrap
        mx-3
        py-2
      ...
      >
        ...
        <v-flex xs12 sm6>
          ...
          <span
            :class="{
              'red--text text--darken-2': userReleasedAlbumsCount == '0',
            }"
            >{{ userReleasedAlbumsCount }}</span
          >
        </v-flex>
        <v-flex xs12 sm6>
          ...
          <span
            :class="{
              'red--text text--darken-2': userYearsSinceFirstReleased == '0',
            }"
            >{{ userYearsSinceFirstReleased }}</span
          >
        </v-flex>
        <v-flex xs12 sm6>
          <label>Plan to run live video?</label>
          <span
            :class="{ 'red--text text--darken-2': !item.will_run_live_video }"
            >{{ getNameById(boolean_options, item.will_run_live_video) }}</span
          >
        </v-flex>
        <v-flex xs12 sm6>
          <label>Do you sell products?</label>
          <span
            :class="{ 'red--text text--darken-2': !item.will_sell_products }"
            >{{ getNameById(boolean_options, item.will_sell_products) }}</span
          >
        </v-flex>
        <v-flex xs12 sm6>
          ...
          <span
            :class="{
              'red--text text--darken-2': !item.will_sell_physical_copies,
            }"
            >{{
              getNameById(boolean_options, item.will_sell_physical_copies)
            }}</span
          >
        </v-flex>
        <v-flex xs12 sm6>
          ...
          <span
            :class="{
              'red--text text--darken-2':
                userAnnualIncomeOnMerchSales == 'None',
            }"
            >{{ userAnnualIncomeOnMerchSales }}</span
          >
        </v-flex>
        <v-flex xs12 sm6>
        ...
        <v-flex xs12 sm6>
          ...
          <span
            :class="{ 'red--text text--darken-2': item.signed_status == 'No' }"
            >{{ item.signed_status }}</span
          >
        </v-flex>
        <v-flex xs12 sm6>
      ...
      <v-layout
        v-else-if="item.request_role == 'label'"
        row
        wrap
        mx-3
        py-2
      ...
      >
        ...
        <v-flex xs12 sm6>
          ...
          <span
            :class="{
              'red--text text--darken-2': !item.is_business_registered,
            }"
            >{{
              getNameById(boolean_options, item.is_business_registered)
            }}</span
          >
        </v-flex>
        <v-flex xs12 sm6>
          ...
          <span
            :class="{ 'red--text text--darken-2': userArtistsCount == 'None' }"
            >{{ userArtistsCount }}</span
          >
        </v-flex>
      </v-layout>
      <v-layout
        v-else-if="item.request_role == 'brand'"
        row
        wrap
        mx-3
        py-2
      ...
      >
        <v-flex xs12 sm6>
          ...
          <span>{{
            item.standard_brand_type || item.customized_brand_type
          }}</span>
        </v-flex>
        ...
        <v-flex xs12 sm6>
          ...
          <span
            :class="{ 'red--text text--darken-2': userYearsInBusiness == '0' }"
            >{{ userYearsInBusiness }}</span
          >
        </v-flex>
        <v-flex xs12 sm6>
        ...
        <v-flex xs12 sm6>
          ...
          <span
            :class="{ 'red--text text--darken-2': userProductsCount == '0' }"
            >{{ userProductsCount }}</span
          >
        </v-flex>
        <v-flex xs12 sm6>
          <label>Annual Income:</label>
          <span
            :class="{ 'red--text text--darken-2': userAnnualIncome == 'None' }"
            >{{ userAnnualIncome }}</span
          >
        </v-flex>
        <v-flex xs12 sm6>
          <label>Business registered?</label>
