...
export default {
  ...
  methods: {
    ...
    selectItemIndex(index) {
      ...
      const itemId = (this.request_tab === 'album'
        ? this.albums
    ...
    updateSettings() {
      if (
        this.room.settings.charLimitBool &&
        typeof this.rules.number(this.room.settings.charLimit) === 'string'
      ) {
        return
      }
      if (
        typeof this.room.settings.charLimit === 'string' &&
        this.room.settings.charLimit !== ''
      ) {
        this.room.settings.charLimit = parseInt(this.room.settings.charLimit)
      }
    ...
    loadPage() {
      ...
      UserService.getUserInfo(this.artist)
        .then((response) => {
          ...
          this.chat_socket = new SocketManager(
            ...
            () => {
              ...
              this.chat_socket.onRoomInfo = async (room) => {
                ...
                let temponlineUsers = await Promise.all(
                  _.chain(room.online)
                    .compact()
                    .map(async (username) => {
                ...
                let tempidleUsers = await Promise.all(
                  room.idle.map(async (username) => {
                    const res = await UserService.getUserInfo(username)
                    return res.body
                  })
                )
                vm.adminUsers = await Promise.all(
                  room.admins.map(async (username) => {
                    const res = await UserService.getUserInfo(username)
                    return res.body
                  })
                )
                ...
                Vue.set(
                  vm,
                  'idleUsers',
                  tempidleUsers.filter((u) => {
                    return room.admins.indexOf(u.username) < 0
                  })
                )
                vm.admin =
                  room.admins.filter((u) => {
              ...
              this.chat_socket.onLoadMessages = function (loadMessageObj) {
                ...
                vm.connected = true
                vm.messageError = ''
                $('#msg-container').scrollTop(
                  $('#msg-container').prop('scrollHeight')
                )
