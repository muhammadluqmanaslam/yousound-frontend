...
export default {
	...
	methods: {
		...
		async handleNextStage() {
			...
			const regularExpression = /^[A-Za-z0-9_.]{3,20}$/
			if (this.username == '' ||  !(regularExpression.test(this.username))) {
				this.$store.dispatch("error/showErrorToast", ["Username must be between 3 to 20 characters without special characters in it"])
			} else {
