            ...
            <template v-if="acceptButtonFunc">
              ...
              <div class="label-actions-section">
                <v-btn
                  ...
                  @click.native="acceptButtonFunc(album, user)"
                  >Accept</v-btn
                >
                <v-btn
                  dark
                  class="action-btn deny"
                  @click.native="denyButtonFunc(album, user)"
                  >Deny</v-btn
                >
              </div>
            </template>
            <template v-else>
              <v-btn dark class="action-btn" v-if="status && status != ''">{{
                status
              }}</v-btn>
              ...
              <v-btn
                ...
                @click.native="deleteButtonFunc(album, user)"
                v-if="deleteButtonFunc"
                ><span v-html="deleteButtonText"></span
              ></v-btn>
            </template>
          </v-flex>
          <p class="album-count" v-if="!acceptButtonFunc && album.tracks">
            {{ album.tracks }} tracks
          </p>
        </v-flex>
      </v-flex>
