...
export default {
  ...
  computed: {
    ...
    pending_labels () {
      return _.filter(this.labels, (item) => { return item.status === 'pending' })
    },
  ...
  methods: {
    ...
    loadData () {
      ...
      Promise.all([
        LabelService.getLabelUsers(),
        LabelService.getLabelAlbums()
      ]).then(values => {
    ...
    acceptLabelUserRequest (host) {
      UserService.acceptLabelRequest(host.id).then(response => {
        this.loadData()
        this.navigatorState.tab = 'approved_labels'
        this.$store.dispatch('navigator/goNextState', _.cloneDeep(this.navigatorState))
      })
    },
    ...
    sendRequestToAdd(artist) {
      UserService.sendLabelRequest(artist.id).then(res => {
    ...
    removeLabel () {
      UserService.removeLabel(this.user.id).then(response => {
        this.loadData()
    ...
    removeAlbum () {
      ...
      AlbumService.removeLabel(this.album.id, params).then(response => {
        _.remove(this.albums, (item) => { return item.album_id == this.album.id && item.user_id == label_id});
